      ...
      <div className="mx-auto flex h-20 max-w-[1480px] items-center justify-between px-5 sm:px-8 lg:px-14">
        <a href="#home" className="flex items-center gap-3" aria-label="Driftwear Clo. home">
          ...
          <span className="leading-none">
            <span className="block text-sm font-black uppercase tracking-[.2em] text-white">Driftwear</span>
            <span className="block text-xs font-black uppercase tracking-[.24em] text-gold">Clo.</span>
          </span>
        </a>

        <nav className="hidden items-center gap-8 lg:flex" aria-label="Main navigation">
          {navItems.map((item) => (
            <a key={item.href} href={item.href} className="text-xs font-black uppercase tracking-[.18em] text-white/65 transition hover:text-gold">
              {item.label}
            </a>
          ))}
        </nav>
      ...
      {open ? (
        <div className="border-t border-white/10 bg-obsidian px-5 py-5 lg:hidden">
          <nav className="grid gap-2" aria-label="Mobile navigation">
            {navItems.map((item) => (
              <a key={item.href} href={item.href} onClick={() => setOpen(false)} className="rounded-2xl border border-white/10 bg-white/[.04] px-4 py-4 text-sm font-black uppercase tracking-[.16em]">
                {item.label}
              </a>
            ))}
